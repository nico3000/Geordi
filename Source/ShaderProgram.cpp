...
bool ShaderProgram::Load(LPCSTR p_file, LPCSTR p_functionVS, LPCSTR p_functionGS, LPCSTR p_functionPS, GraphicsLayer::ShaderVersion p_version /* = GraphicsLayer::SHADER_VERSION_MAX */)
{
    if(p_functionVS != 0)
    {
        m_pVS = LostIsland::g_pGraphics->CompileVertexShader(p_file, p_functionVS, m_pVSBlob, p_version);
        if(m_pVS == 0)
        {
            return false;
        }
    ...
    if(p_functionGS != 0)
    {
        m_pGS = LostIsland::g_pGraphics->CompileGeometryShader(p_file, p_functionGS, p_version);
        if(m_pGS == 0)
        {
            return false;
        }
    ...
    if(p_functionPS != 0)
    {
        m_pPS = LostIsland::g_pGraphics->CompilePixelShader(p_file, p_functionPS, p_version);
        if(m_pPS == 0)
        {
            return false;
        }
